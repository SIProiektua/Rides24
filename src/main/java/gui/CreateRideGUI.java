...
public class CreateRideGUI extends JFrame {
	...
	public CreateRideGUI(Driver driver) {
		...
		JComboBox<Car> comboBoxCar = new JComboBox<Car>();

		DefaultComboBoxModel<Car> kotxeenMatrikula = new DefaultComboBoxModel<Car>();
		// selects all driver's car's plate numbers.
		kotxeenMatrikula.addAll(driver.getCars());
		comboBoxCar.setModel(kotxeenMatrikula);
		comboBoxCar.setSelectedIndex(0);
		...
		if (error != null)
			...
			try {
				...
				Ride r = facade.createRide(fieldOrigin.getText(), fieldDestination.getText(),
						UtilDate.trim(jCalendar.getDate()), inputSeats, price, driver.getEmail(), car);
