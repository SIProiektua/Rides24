...
public class DataAccess {
	...
	public boolean getUser(String u) {
		TypedQuery<User> query = db.createQuery("SELECT DISTINCT r.username FROM User r WHERE r.username =?1",
				User.class);
	...
	public boolean getPass(String user, String pass) {
		TypedQuery<User> query = db.createQuery(
				"SELECT DISTINCT r.password FROM User r WHERE r.password =?1 and r.username=?2", User.class);
	...
	//LO de que aparezca ane gaztañaga es que creo q es culpa de que pusimos para que reconociera la errenkada creo q tenemos que ir seleccionando todo por rows pero con la misma columna
	/**
